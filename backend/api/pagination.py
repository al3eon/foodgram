...
class CustomLimitOffsetPagination(LimitOffsetPagination):
    ...
    def get_paginated_response(self, data):
        ...
        if self.count > offset + limit:
            next_page = f"{self.request.build_absolute_uri(self.request.path)}?limit={limit}&offset={offset + limit}"
        previous_page = None
        if offset > 0:
            previous_page = f"{self.request.build_absolute_uri(self.request.path)}?limit={limit}&offset={max(offset - limit, 0)}"
        return Response({
            'count': self.count,
            'next': next_page,
